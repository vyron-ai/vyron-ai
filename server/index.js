...
async function probeVideo(filePath) {
  ...
  const container =
    EXT_MAP[ext] ?? (fmtLine?.[1] ?? "unknown").trim().split(",")[0];
...
const ASS_PRESET = {
  viral: {
    ...
    styleDef:
      "Arial,22,&H80FFFFFF,&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,3,1,2,20,20,55,1",
  },
  documentary: {
    ...
    styleDef:
      "Arial,17,&H9EFFFFFF,&H000000FF,&H00000000,&H80000000,0,0,0,0,100,100,0,0,3,1,0,2,20,20,45,1",
  },
  podcast: {
    ...
    styleDef:
      "Arial,17,&H80FFFFFF,&H000000FF,&H00000000,&H90000000,0,0,0,0,100,100,0,0,3,1,0,2,20,20,45,1",
  },
};
...
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text`;
  ...
  for (const seg of subtitles) {
    ...
    const lineOffset =
      estimatedLines === 3 ? 128 : estimatedLines === 2 ? 64 : 0;
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      const slotEnd =
        i < words.length - 1 ? seg.start + (i + 1) * slotMs : seg.end;
...
app.post("/api/content-planner/generate", (req, res) => {
  ...
  const objectiveMapEN = {
    ...
    lead_generation: [
      "Drive link-in-bio clicks",
      "Generate DM inquiries",
      "Capture email subscribers",
      "Build prospect list",
    ],
    sales: [
      "Drive offer conversions",
      "Present product value",
      "Overcome objections",
      "Move warm leads to action",
    ],
    engagement: [
      "Maximize comments",
      "Encourage saves & shares",
      "Build community conversation",
      "Boost interaction rate",
    ],
    community: [
      "Deepen audience relationship",
  ...
  const objectiveMapES = {
    brand_awareness: [
      ...
      "Crecer seguidores orgánicos",
      "Construir autoridad en el nicho",
      "Aumentar recordación de marca",
    ],
    lead_generation: [
      "Generar clics en el enlace de bio",
      "Conseguir consultas por DM",
      "Capturar suscriptores de email",
      "Construir lista de prospectos",
    ],
    sales: [
      "Impulsar conversiones de oferta",
      "Presentar el valor del producto",
      "Superar objeciones",
      "Mover prospectos tibios a acción",
    ],
    engagement: [
      "Maximizar comentarios",
      "Fomentar guardados y compartidos",
      "Generar conversación en comunidad",
      "Aumentar tasa de interacción",
    ],
    community: [
      "Profundizar relación con audiencia",
      "Fomentar conversación bidireccional",
      "Crear defensores de marca",
  ...
  const ctaTemplatesEN = {
    curiosity: [
      "Comment 'HOW' and I'll send the full breakdown",
      "Follow for the answer in part 2",
      "Save this before it disappears",
    ],
    pain: [
      "Comment 'STUCK' if this is you right now",
      "DM me — I'll show you the first step",
      "Link in bio if you're ready to fix this",
    ],
    story: [
      "Comment 'ME TOO' if you've been here",
    ...
    authority: [
      ...
      "Comment 'FRAMEWORK' for the PDF version",
    ],
    mistake: [
      "Comment 'GUILTY' if you've done this",
      "Follow — I share the fix every week",
      "Save this before you make the same mistake",
    ],
    opportunity: [
      "Comment 'IN' if you want the details",
      "Link in bio — the window won't stay open",
      "Follow to catch the next opportunity early",
    ],
    viral: [
      "Share this with one creator who needs it",
      "Follow for what's working right now",
      "Duet or stitch this — let me know your take",
    ],
  };

  const ctaTemplatesES = {
    curiosity: [
      `Comenta 'CÓMO' y te mando el desglose completo de cómo conseguir ${tr.practicalOutcome}`,
      "Sígueme para la respuesta en la parte 2",
      "Guarda esto antes de que desaparezca",
    ],
    pain: [
      `Comenta 'LISTO' si quieres ${tr.practicalOutcome} y todavía no lo tienes`,
      `Escríbeme por DM — te muestro el primer paso hacia ${tr.emotionalTransformation}`,
      "Enlace en bio si estás listo para solucionar esto",
    ],
    story: [
      `Comenta 'YO TAMBIÉN' si todavía no tienes ${tr.practicalOutcome}`,
      "Sígueme para el próximo capítulo",
      "Comparte esto con alguien que lo necesita",
    ],
    authority: [
      `Guarda esto si quieres ${tr.practicalOutcome} de forma consistente`,
      "Sígueme si quieres el desglose completo",
      `Comenta '${n.toUpperCase()}' para los detalles`,
    ],
    mistake: [
      `Comenta 'CULPABLE' si esto te ha impedido ${tr.practicalOutcome}`,
      `Sígueme — publico cómo conseguir ${tr.emotionalTransformation} cada semana`,
      "Guarda esto antes de cometer el mismo error",
    ],
    opportunity: [
      `Comenta 'DENTRO' si quieres ${tr.practicalOutcome} antes de que se cierre la ventana`,
      "Enlace en bio — la ventana no va a quedar abierta",
      "Sígueme para detectar la próxima oportunidad temprano",
    ],
    viral: [
      `Comparte esto con alguien que quiere ${tr.practicalOutcome} en ${n}`,
      `Sígueme para lo que realmente entrega ${tr.emotionalTransformation} ahora`,
      "Duplícalo o une — dime tu perspectiva",
    ],
  };
  ...
  for (let i = 0; i < total; i++) {
    const dayNum =
      total === 1 ? 1 : Math.round((i / (total - 1)) * (dur - 1)) + 1;
...
const _INDUSTRY_MAP = [
  {
    ...
    es: {
      ...
      desires:
        "proyectar confianza, causar una primera impresión impactante y verse al nivel de quien ya son por dentro",
      fears:
        "verse descuidados, no causar buena impresión o que su apariencia los ponga en desventaja antes de abrir la boca",
      pains:
        "la desconexión entre cómo se sienten por dentro y cómo se presentan al mundo — una imagen que no refleja su nivel real",
      transformation:
        "de verse del montón a proyectar autoridad visual — de invisible a memorable, de descuidado a siempre impecable",
    },
    en: {
      ...
      desires:
        "to project confidence, make a powerful first impression and look like the person they already are on the inside",
      fears:
        "looking unkempt, making a bad first impression or letting their appearance put them at a disadvantage",
      pains:
        "the gap between how they feel inside and how they present to the world — an image that doesn't match their actual level",
      transformation:
        "from blending in to projecting real authority — from overlooked to unforgettable, from inconsistent to always sharp",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "un físico que refleje su esfuerzo, hábitos que funcionen sin fuerza de voluntad heroica y el progreso visible que los mantenga motivados",
      fears:
        "estancarse, perder el progreso acumulado o dedicar meses de esfuerzo sin ver resultados reales",
      pains:
        "la inconsistencia que sabotea el progreso — empezar bien y abandonar, o esforzarse sin ver el físico que deberían tener",
      transformation:
        "de inconsistente a imparable — de empezar y abandonar a construir el físico y los hábitos que se sostienen solos",
    },
    en: {
      ...
      desires:
        "a physique that reflects their effort, habits that work without heroic willpower, and visible progress that keeps them motivated",
      fears:
        "plateauing, losing all accumulated progress, or putting in months of effort without seeing real results",
      pains:
        "the inconsistency that sabotages any progress — starting strong and falling off, or grinding without seeing the results they should",
      transformation:
        "from inconsistent to unstoppable — from starting-and-quitting to building the physique and habits that sustain themselves",
    },
  },
  {
    ...
    es: {
      ...
      phrases: [
        ...
        "mejorar tu sonrisa",
        "sentirte seguro al sonreír",
        "salud dental impecable",
      ],
      desires:
        "sonreír sin inhibiciones, sentirse seguros en cualquier conversación y tener una sonrisa que los favorezca, no que los frene",
      fears:
        "que su sonrisa los haga sentir inseguros, que los demás lo noten o que un problema menor se convierta en uno grave",
      pains:
        "evitar sonreír libremente, cubrir la boca al reír o sentir que su sonrisa no refleja quiénes son en realidad",
      transformation:
        "de esconder la sonrisa a mostrarla sin pensarlo — de inseguridad e incomodidad a confianza plena en cada conversación",
    },
    en: {
      synonyms: [
        "oral health",
      ...
      phrases: [
        ...
        "improve your smile",
        "feel secure when smiling",
        "flawless dental health",
      ],
      desires:
        "to smile without inhibition, feel confident in any conversation and have a smile that works for them, not against them",
      fears:
        "that their smile makes them self-conscious, that others notice it, or that a small problem becomes a serious one",
      pains:
        "hiding their smile, covering their mouth when laughing, or feeling like their smile doesn't reflect who they really are",
      transformation:
        "from hiding their smile to showing it without thinking — from insecurity to full confidence in every interaction",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "un local lleno de clientes fieles que vuelven y refieren — no solo por la comida, sino por la experiencia que solo ellos ofrecen",
      fears:
        "mesas vacías, clientes que no regresan o que la competencia los opaque sin entender por qué",
      pains:
        "atraer clientes una vez pero no lograr que vuelvan — invertir en publicidad sin ver el retorno en ventas reales",
      transformation:
        "de depender del volumen a tener una base de clientes fieles — de transacciones a una comunidad gastronómica real",
    },
    en: {
      synonyms: [
      ...
      desires:
        "a full house of loyal regulars who return and refer — not just for the food, but for the experience only they offer",
      fears:
        "empty tables, one-time visitors who never return, or competitors outshining them without understanding why",
      pains:
        "attracting customers once but failing to bring them back — spending on ads without seeing it return in real revenue",
      transformation:
        "from volume-dependent to a loyal customer base — from transactions to a real dining community",
    },
  },
  {
    // ── Abogado / Legal ──────────────────────────────────────────────────────
    ...
    es: {
      ...
      phrases: [
        ...
        "resolver sin complicaciones",
        "dormir tranquilo",
      ],
      desires:
        "certeza jurídica, protección real de lo que han construido y la tranquilidad de saber que están blindados ante cualquier riesgo",
      fears:
        "perder lo que han construido por un error legal evitable o enfrentar una situación crítica sin la preparación necesaria",
      pains:
        "la incertidumbre de no saber si están legalmente protegidos — operar con riesgo invisible o actuar tarde ante un problema crítico",
      transformation:
        "de expuestos a protegidos — de operar con incertidumbre jurídica a tener certeza total sobre lo que construyen y cómo lo defienden",
    },
    en: {
      synonyms: [
        "legal advisory",
      ...
      desires:
        "legal certainty, real protection of what they've built, and the peace of mind of knowing they're fully shielded against any risk",
      fears:
        "losing what they've built through an avoidable legal mistake, or facing a critical situation without proper preparation",
      pains:
        "not knowing if they're legally protected — operating with invisible risk or acting too late when a problem becomes critical",
      transformation:
        "from exposed to protected — from operating with legal uncertainty to having total clarity over what they're building and how it's defended",
    },
  },
  {
    ...
    es: {
      ...
      phrases: [
        ...
        "generar ventas predecibles",
      ],
      desires:
        "un flujo predecible de clientes calificados, posicionamiento claro y un sistema de ventas que funcione sin depender de su tiempo",
      fears:
        "invertir en publicidad sin retorno, quedar invisible mientras la competencia crece o no saber qué está fallando en la estrategia",
      pains:
        "tiempo y dinero invertido en marketing sin resultados claros — activos en redes pero sin clientes ni crecimiento real",
      transformation:
        "de invisible a referente — de publicar sin rumbo a tener un sistema que atrae, convierte y retiene clientes de forma sistemática",
    },
    en: {
      synonyms: [
      ...
      desires:
        "a predictable flow of qualified clients, clear positioning, and a sales system that works without depending on their time",
      fears:
        "investing in ads without return, staying invisible while competitors grow, or not knowing what's broken in the strategy",
      pains:
        "time and money in marketing with no clear results — active online but not generating clients or real growth",
      transformation:
        "from invisible to reference — from posting aimlessly to a system that attracts, converts and retains clients systematically",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "patrimonio que se valorice, ingresos pasivos que den libertad y la certeza de haber tomado la decisión correcta antes de que sea tarde",
      fears:
        "comprar mal, perder dinero en una inversión que parecía segura o que el mercado los deje atrás mientras otros construyen patrimonio",
      pains:
        "la parálisis por análisis — saber que el ladrillo es buena inversión pero no tener claridad sobre cuándo, dónde ni cómo entrar correctamente",
      transformation:
        "de ahorrador pasivo a inversionista activo — de dinero inactivo a activos que crecen y generan rentabilidad predecible",
    },
    en: {
      synonyms: [
      ...
      desires:
        "wealth that appreciates, passive income that gives freedom, and certainty of having made the right move at the right time",
      fears:
        "buying wrong, losing money on an investment that seemed safe, or being left behind while others build wealth",
      pains:
        "analysis paralysis — knowing real estate is a good investment but lacking clarity on when, where and how to enter correctly",
      transformation:
        "from passive saver to active investor — from idle money to assets that grow and generate predictable returns",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "transformación real y medible, claridad sobre su camino y la certeza de estar avanzando sistemáticamente hacia su mejor versión",
      fears:
        "invertir en formación y no aplicarla, quedarse en el mismo punto después de haberlo intentado todo o ser la excepción",
      pains:
        "saber qué necesitan cambiar pero no saber cómo — tener motivación sin el sistema que la convierta en resultados consistentes",
      transformation:
        "de saber pero no aplicar a ejecutar con claridad — de potencial sin activar a crecimiento medible y sostenido semana a semana",
    },
    en: {
      synonyms: [
        "coaching",
      ...
      desires:
        "real and measurable transformation, clarity on their path, and the certainty they're consistently moving toward their best version",
      fears:
        "investing in training and not applying it, staying in the same place after trying everything, or being the exception",
      pains:
        "knowing what they need to change but not knowing how — having motivation without the system to convert it into consistent results",
      transformation:
        "from knowing but not applying to executing with clarity — from untapped potential to measurable, sustained growth every week",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "libertad financiera real — no preocuparse por el dinero, generar ingresos sin intercambiar tiempo y tener certeza sobre el futuro económico",
      fears:
        "trabajar toda la vida sin construir riqueza, tomar malas decisiones financieras o depender de un ingreso que podría desaparecer",
      pains:
        "ganar dinero pero no acumularlo — ingresos que llegan y se van sin construir el patrimonio que debería seguir de ese esfuerzo",
      transformation:
        "de intercambiar tiempo por dinero a construir activos — de vivir al día a un sistema financiero que trabaja mientras ellos descansan",
    },
    en: {
      ...
      desires:
        "real financial freedom — not worrying about money, generating income without trading time, and certainty about their financial future",
      fears:
        "working their whole life without building wealth, making bad financial decisions, or depending on income that could disappear",
      pains:
        "earning money but not accumulating it — income that comes and goes without building the wealth that should follow from their effort",
      transformation:
        "from trading time for money to building assets — from paycheck to paycheck to a financial system that works while they rest",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "sentirse verdaderamente bien — con energía, equilibrio emocional y la certeza de estar cuidando su salud de forma proactiva",
      fears:
        "que un problema menor se convierta en grave por no atenderlo a tiempo, o que su calidad de vida se deteriore sin poder hacer nada",
      pains:
        "vivir con síntomas que afectan su calidad de vida sin saber exactamente qué los causa ni cómo resolverlos de forma definitiva",
      transformation:
        "de gestionar síntomas a tener salud de verdad — de sobrevivir el día a vivir con la energía y equilibrio que merecen",
    },
    en: {
      ...
      desires:
        "to feel truly well — with energy, emotional balance and the certainty they're caring for their health proactively",
      fears:
        "a small problem becoming serious because they didn't address it in time, or their quality of life deteriorating",
      pains:
        "living with symptoms that affect their quality of life without knowing exactly what causes them or how to resolve them definitively",
      transformation:
        "from managing symptoms to having real health — from surviving the day to living with the energy and balance they deserve",
    },
  },
  {
    ...
    es: {
      ...
      desires:
        "procesos automatizados que liberen su tiempo, capacidad de escalar sin añadir fricción y datos claros para mejores decisiones",
      fears:
        "invertir en tecnología que no se adopta, implementaciones que generan más problemas de los que resuelven o quedar obsoleto rápido",
      pains:
        "procesos manuales que consumen tiempo valioso, sistemas desconectados y decisiones tomadas sin datos reales o con información incompleta",
      transformation:
        "de operación manual a sistemas que trabajan solos — de reactividad y caos operativo a control total con automatización real",
    },
    en: {
      synonyms: [
        "the tech solution",
      ...
      desires:
        "automated processes that free their time, the ability to scale without friction, and clear data for better decisions",
      fears:
        "investing in tech that doesn't get adopted, implementations that create more problems than they solve, or falling behind quickly",
      pains:
        "manual processes consuming valuable time, disconnected systems, and decisions made without real or complete data",
      transformation:
        "from manual operations to systems that run themselves — from reactive chaos to total control with real automation",
    },
  },
  {
    ...
    es: {
      ...
      phrases: [
        ...
        "escalar ingresos de forma predecible",
      ],
      desires:
        "ventas predecibles, compradores que regresan solos y una tienda que genera ingresos sistemáticamente sin atención constante",
      fears:
        "invertir en tráfico que no convierte, acumular inventario sin rotación o competir solo por precio sin poder ganar",
      pains:
        "tráfico sin ventas — visitas que llegan pero no compran, o compradores que no vuelven a pesar de una buena primera experiencia",
      transformation:
        "de tienda activa a motor de ingresos — de picos de tráfico impredecibles a un sistema que convierte y retiene de forma sostenida",
    },
    en: {
      ...
      desires:
        "predictable sales, customers who return on their own, and a store that generates income systematically without constant attention",
      fears:
        "investing in traffic that doesn't convert, inventory piling up, or competing on price without a way to win",
      pains:
        "traffic without sales — visitors who arrive but don't buy, or buyers who never return despite a good first experience",
      transformation:
        "from active store to revenue engine — from traffic spike dependency to a system that consistently converts and retains",
    },
  },
];
...
const _INDUSTRY_FALLBACK = {
  es: {
    ...
    desires:
      "resultados concretos, crecimiento sostenido y la certeza de que lo que hacen funciona de forma predecible",
    fears:
      "invertir esfuerzo sin ver resultados claros o que la competencia avance mientras ellos se estancan",
    pains:
      "la falta de un sistema que genere resultados de forma consistente — mucho esfuerzo, pocos resultados medibles",
    transformation:
      "de esfuerzo sin dirección a un sistema que genera resultados — del azar al control sobre los outcomes",
  },
  en: {
    ...
    desires:
      "concrete results, sustained growth, and the certainty that what they're doing works predictably",
    fears:
      "investing effort without clear results, or watching competitors advance while they stagnate",
    pains:
      "the lack of a system that generates results consistently — a lot of effort, few measurable outcomes",
    transformation:
      "from directionless effort to a system that generates results — from depending on luck to controlling outcomes",
  },
};
...
function industryProfile(n, isES) {
  ...
  for (const ind of _INDUSTRY_MAP) {
    if (ind.match.some((w) => nn.includes(norm(w))))
      return isES ? ind.es : ind.en;
  }
...
function audienceLock(au, n, isES) {
  if (!au) {
    return {
      variants: [isES ? "tu audiencia" : "your audience"],
      subject: "",
      goal: "",
      blacklist: [],
    };
  ...
  const goalRephrases = (profile.phrases || [])
    .slice(0, 5)
  ...
  if (goal) {
    ...
    const goalPool = [
      goal,
      ...goalRephrases.filter((g) => normStr(g) !== normStr(goal)),
    ].slice(0, 4);
...
function ceAuVars(au, n, isES) {
  ...
  return lock.variants.length
    ? lock.variants
    : [au || (isES ? "tu audiencia" : "your audience")];
}
...
function ceNVars(n, isES) {
  if (!n) return [];
  const short =
    n.split(/\s+/).length > 2 ? n.split(/\s+/).slice(0, 2).join(" ") : null;
  const profile = industryProfile(n, isES);
...
function cePrVars(pr, isES) {
  if (!pr) return [];
  const short =
    pr.split(/\s+/).length > 2 ? pr.split(/\s+/).slice(0, 2).join(" ") : null;
  const unique = (arr) => [...new Set(arr.filter(Boolean))];
  return unique(
    isES
      ? [
...
function ctxBuild(n, au, pr, isES, maxPerField = 1, maxGlobal = 3) {
  ...
  function applyOne(text, phrase, vars, gk, vik) {
    ...
    return text.replace(new RegExp(esc, "gi"), () => {
      ...
      const r = vars[s[vik] % vars.length];
      s[vik]++;
      return r || phrase;
    });
  }
...
function buildTransformation(n, pr, isES) {
  ...
  const isAppearance =
    /corte|peluquer|barber|estilis|cabello|pelo|hair|nail|u[ñn]as|makeup|maquill|estetica|spa|aesthetic|lash|pest[aá]n|cejas?|brow|look|imagen personal/.test(
      text,
    );
  const isPhysical =
    /gym|gimnasio|entrena|fitness|nutri|dieta|diet|ejercici|workout|train|weight|peso|musculo|muscle|cardio|hiit|crossfit|tonific/.test(
      text,
    );
  const isHealth =
    /dent|sonris|blanque|whitening|cl[íi]nic|salud|health|limpieza dental|medic|doctor|odonto|mental|terapia|therapy|quiropract/.test(
      text,
    );
  const isExperience =
    /restaur|food|comida|chef|cater|event|experienc|baker|pasteler|coffee|caf[eé]|\bbar\b|cocina|brunch|sushi|pizz|helad/.test(
      text,
    );
  const isBusiness =
    /marketing|publicidad|\bads\b|redes sociales|social media|contenido digital|\bcontent\b|\bbrand\b|agencia|agency|\bseo\b|\bemail\b|digital|ventas|sales|\bcrm\b|ecommerce|tienda/.test(
      text,
    );
  const isLegal =
    /abogad|lawyer|legal|contable|contador|account|consultor|asesor|auditor|fiscal|seguro|insurance|notari/.test(
      text,
    );
  const isEducation =
    /curso|course|coach|mentor[íi]a|mentor|escuela|school|academia|workshop|taller|certific|aprender|learn|formaci/.test(
      text,
    );
  const isProperty =
    /inmobi|real estate|\bcasa\b|piso|apartamento|propert|arrendar|renta|alquiler/.test(
      text,
    );
  ...
  if (isES) {
    if (isAppearance)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "verse bien y causar una primera impresión que no pide disculpas",
        emotionalTransformation:
          "seguridad, orgullo propio y sensación de control sobre su imagen",
        socialTransformation:
          "respeto inmediato, autoridad visual, ser recordado por cómo se presenta",
        practicalOutcome:
          "apariencia mejorada, presencia más fuerte, imagen profesional",
        fearAvoided:
          "verse descuidado o invisible — perder oportunidades por mala presentación",
        identityShift:
          "alguien que cuida su imagen con intención — visible en cada contexto",
      };
    if (isPhysical)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "disciplina real, un cuerpo diferente, energía constante y atractivo renovado",
        emotionalTransformation:
          "confianza, fuerza mental y la sensación de poder sobre sí mismo",
        socialTransformation:
          "presencia física notable, reconocimiento por el cambio visible, atractivo renovado",
        practicalOutcome:
          "composición corporal mejorada, energía sostenida, rendimiento físico superior",
        fearAvoided:
          "verse igual en un año — desperdiciar el potencial físico que ya tienen",
        identityShift:
          "alguien disciplinado — la versión física que siempre supieron que podían ser",
      };
    if (isHealth)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "sonrisa segura, confianza social y sentirse saludable y atractivo",
        emotionalTransformation:
          "seguridad, ligereza y ganas de mostrarse sin pensarlo dos veces",
        socialTransformation:
          "primera impresión limpia, accesibilidad, bienestar que se nota a simple vista",
        practicalOutcome:
          "salud mejorada, apariencia cuidada y sensación de bienestar real",
        fearAvoided:
          "vergüenza en situaciones sociales — una apariencia que apaga su presencia",
        identityShift:
          "alguien que cuida su salud y lo demuestra — sin excusas ni postergaciones",
      };
    if (isExperience)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "un momento memorable, conexión real y sabor que queda en la memoria",
        emotionalTransformation:
          "placer, bienestar y nostalgia positiva que quieren repetir",
        socialTransformation:
          "compartir algo especial, impresionar a quien invitan, crear recuerdos juntos",
        practicalOutcome:
          "experiencia superior, recuerdos creados, momento que realmente vale la pena",
        fearAvoided:
          "gastar en algo olvidable — decepcionar a quien llevan consigo",
        identityShift:
          "alguien que sabe elegir experiencias que realmente valen la pena",
      };
    if (isBusiness)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "clientes que llegan solos, ventas predecibles y visibilidad que posiciona",
        emotionalTransformation:
          "confianza en su negocio, claridad y alivio de saber que el sistema funciona",
        socialTransformation:
          "autoridad de marca, reconocimiento en su mercado, liderazgo percibido",
        practicalOutcome:
          "más clientes, más ventas y alcance que crece con cada publicación",
        fearAvoided:
          "quedar invisible mientras competidores capturan la atención que merecen ellos",
        identityShift:
          "un negocio que atrae — que no persigue clientes, sino que los recibe",
      };
    if (isLegal)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "protección real, paz mental y certeza de que lo construido no está en riesgo",
        emotionalTransformation:
          "tranquilidad, control y la sensación de tener las bases bien puestas",
        socialTransformation:
          "credibilidad, seriedad percibida y confianza que proyectan ante otros",
        practicalOutcome:
          "riesgo minimizado, procesos en orden y cumplimiento garantizado",
        fearAvoided:
          "perder lo construido — multas, problemas legales o fiscales evitables",
        identityShift:
          "alguien que opera con seriedad — cubierto, protegido, en orden",
      };
    if (isEducation)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "habilidades reales, resultados medibles y la versión más capaz de sí mismos",
        emotionalTransformation:
          "confianza, claridad y orgullo de dominar algo que antes parecía lejano",
        socialTransformation:
          "reconocimiento por su progreso y autoridad en lo que saben hacer",
        practicalOutcome:
          "habilidades aplicables, avance real y certificación que los respalda",
        fearAvoided:
          "invertir tiempo y dinero en formación que no cambia nada concreto",
        identityShift:
          "alguien que invirtió en sí mismo — y tiene resultados para demostrarlo",
      };
    if (isProperty)
      return {
        visibleProduct: pr || n,
        hiddenDesire:
          "estabilidad, un lugar propio y la sensación de haber llegado a algo sólido",
        emotionalTransformation:
          "seguridad, orgullo y la tranquilidad de tener algo que les pertenece",
        socialTransformation:
          "estatus, independencia y ser propietario de algo que otros respetan",
        practicalOutcome:
          "propiedad propia, inversión sólida y espacio que refleja quiénes son",
        fearAvoided:
          "seguir pagando renta sin construir nada — dinero que se va sin retorno",
        identityShift:
          "alguien que tomó la decisión que la mayoría solo habla — y tiene el título para probarlo",
      };
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "resultados reales, claridad y la certeza de que su inversión vale",
      emotionalTransformation:
        "confianza, alivio y la sensación de avanzar en la dirección correcta",
      socialTransformation:
        "reconocimiento, credibilidad y posicionamiento entre quienes los rodean",
      practicalOutcome:
        "resultados visibles, progreso medible y retorno real sobre su tiempo",
      fearAvoided:
        "invertir tiempo y dinero y quedar en el mismo lugar — sin avance visible",
      identityShift:
        "alguien que tomó acción cuando importaba — y tiene el progreso para mostrarlo",
    };
  }
  ...
  if (isAppearance)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "looking great and making a first impression that doesn't need to apologize",
      emotionalTransformation:
        "confidence, self-pride and control over how they present themselves",
      socialTransformation:
        "immediate respect, visual authority, being remembered for how they show up",
      practicalOutcome:
        "improved appearance, stronger presence, professional-looking image",
      fearAvoided:
        "looking careless or forgettable — losing opportunities because of poor presentation",
      identityShift:
        "someone who takes care of their image intentionally — visible in every room",
    };
  if (isPhysical)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "real discipline, a different body, sustained energy and renewed attractiveness",
      emotionalTransformation:
        "confidence, mental strength and the feeling of power over themselves",
      socialTransformation:
        "physical presence, recognition for visible change, renewed attractiveness",
      practicalOutcome:
        "improved body composition, sustained energy, real physical performance",
      fearAvoided:
        "looking the same in a year — wasting the physical potential they already have",
      identityShift:
        "someone disciplined — the physical version of themselves they always knew they could be",
    };
  if (isHealth)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "a confident smile, social ease and feeling healthy and attractive",
      emotionalTransformation:
        "confidence, lightness and showing up without thinking twice",
      socialTransformation:
        "clean first impression, approachability, visible wellbeing",
      practicalOutcome:
        "improved health, cared-for appearance and real sense of wellbeing",
      fearAvoided:
        "social embarrassment — an appearance that dims their presence",
      identityShift:
        "someone who takes care of their health and shows it — no excuses",
    };
  if (isExperience)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "a memorable moment, real connection and flavors that stay with them",
      emotionalTransformation:
        "pleasure, comfort and positive nostalgia they want to repeat",
      socialTransformation:
        "sharing something special, impressing those they invite, creating memories together",
      practicalOutcome:
        "superior experience, memories created, moment that truly delivers",
      fearAvoided:
        "spending on something forgettable — disappointing the people they bring",
      identityShift:
        "someone who knows how to choose experiences that are genuinely worth it",
    };
  if (isBusiness)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "clients who arrive on their own, predictable sales and visibility that positions",
      emotionalTransformation:
        "confidence in their business, clarity and the relief of knowing the system works",
      socialTransformation:
        "brand authority, recognition in their market, perceived leadership",
      practicalOutcome:
        "more clients, more sales and reach that grows with every post",
      fearAvoided:
        "staying invisible while competitors capture the attention that should be theirs",
      identityShift:
        "a business that attracts — that doesn't chase clients, but receives them",
    };
  if (isLegal)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "real protection, peace of mind and certainty that what they've built is secure",
      emotionalTransformation:
        "calm, control and the feeling of having solid foundations",
      socialTransformation:
        "credibility, perceived seriousness and trust they project to others",
      practicalOutcome:
        "risk minimized, processes in order and compliance guaranteed",
      fearAvoided:
        "losing what they've built — fines, legal or tax issues they could have prevented",
      identityShift:
        "someone who operates seriously — covered, protected, in order",
    };
  if (isEducation)
    return {
      visibleProduct: pr || n,
      hiddenDesire:
        "real skills, measurable results and the most capable version of themselves",
      emotionalTransformation:
        "confidence, clarity and pride in mastering something that once felt out of reach",
      socialTransformation:
        "recognition for their progress and authority in what they know how to do",
      practicalOutcome:
        "applicable skills, real advancement and credentials that back them up",
      fearAvoided: "investing time and money in training that changes nothing",
      identityShift:
        "someone who invested in themselves — and has results to prove it",
    };
  return {
    visibleProduct: pr || n,
    hiddenDesire:
      "real results, clarity and certainty that their investment delivers",
    emotionalTransformation:
      "confidence, relief and the feeling of moving in the right direction",
    socialTransformation:
      "recognition, credibility and positioning among those around them",
    practicalOutcome:
      "visible results, measurable progress and real return on their time",
    fearAvoided:
      "investing time and money and ending up in the same place — no visible progress",
    identityShift:
      "someone who took action when it mattered — and has the progress to show for it",
  };
}
...
function buildStagePlannerData(stage, n, pr, au, isES) {
  ...
  if (isPrincipiante)
    return {
      ...
      objectivePool: [
        "Build initial confidence with first clients",
        "Reduce barrier to first result",
        "Create clarity around first concrete step",
        "Land first client with simple action today",
      ],
      topicMap: {
        curiosity: [
          `Why getting your first ${n} client is simpler than it looks`,
          `The step nobody explains when starting ${n} from scratch`,
          `What you actually need to start ${n} (not what you think)`,
          `The clarity secret to start ${n} today without experience`,
        ],
        pain: [
          `Why you still don't have your first ${n} client — the simple fix`,
          `The clarity block that paralyzes beginners in ${n}`,
          `Still no clients? The most common reason when starting ${n}`,
          `Why starting ${n} feels hard when it actually isn't`,
        ],
        story: [
          `How I got my first ${n} clients without prior experience`,
          `From zero to first ${n} clients: what actually worked`,
          `My first mistake in ${n} — and what I learned about clarity`,
          `The first ${n} result that gave me confidence to continue`,
        ],
        authority: [
          `The simplest guide to start ${n} and get your first clients`,
          `What people who land their first ${n} client fast do differently`,
          `The first-client ${n} process that works from day one`,
          `How to build initial trust with your first ${n} clients`,
        ],
        mistake: [
          `The #1 mistake beginners make in ${n} that delays the first client`,
          `This thing beginners do in ${n} kills initial confidence`,
          `The most common ${n} decision that delays first results`,
          `Stop doing this in ${n} — it's costing you clarity`,
        ],
        opportunity: [
          `The window to land your first ${n} client is open right now`,
          `Why now is the best time to start ${n} with clarity`,
          `The ${n} angle for first clients your competition isn't using`,
          `The first-result opportunity in ${n} beginners don't see yet`,
        ],
        viral: [
          `The first step in ${n} all beginners are starting to use`,
          `Why starting ${n} with this approach changes first results`,
          `Everyone's talking about starting ${n} — here's what actually gives first clients`,
          `The mindset shift for starting ${n} giving the most clarity right now`,
        ],
      },
    };
  ...
  if (isPequena)
    return {
      ...
      objectivePool: [
        "Position brand in local niche",
        "Build systematic lead capture",
        "Establish repeatable content process",
        "Strengthen reputation and differentiation",
      ],
      topicMap: {
        curiosity: [
          `The content system that positions your ${n} without heroic effort`,
          `What separates a ${n} with solid reputation from one that can't capture leads`,
          `Why your ${n} needs a capture process, not more random posts`,
          `The ${n} content calendar that captures prospects every week`,
        ],
        pain: [
          `Why your ${n} has visibility but no real lead capture`,
          `Why your ${n} works sometimes but not consistently`,
          `The positioning problem ${n} businesses with potential have without a system`,
          `Inconsistent ${n} reputation? Here's the system that fixes it`,
        ],
        story: [
          `How I built a content system that positions my ${n} consistently`,
          `The ${n} calendar that turned my operation into a repeatable process`,
          `How I went from random results to systematic lead capture in ${n}`,
          `What I changed in my ${n} to have reputation without depending on me`,
        ],
        authority: [
          `The ${n} content system that runs without depending on you`,
          `How to design a repeatable capture process for your ${n}`,
          `What ${n} businesses with solid reputation and consistent leads do differently`,
          `The ${n} positioning calendar that works 7 days a week`,
        ],
        mistake: [
          `The content mistake ${n} businesses without lead systems make`,
          `Why your ${n} doesn't have consistent leads even though you create content`,
          `Why your ${n} still doesn't have solid local reputation`,
          `Stop posting without a calendar — it's killing your ${n} positioning`,
        ],
        opportunity: [
          `The ${n} positioning window small businesses aren't using`,
          `Why now is the time to build your ${n} lead capture system`,
          `The ${n} differentiation angle your competition isn't using`,
          `The ${n} reputation opportunity for small businesses nobody's taking`,
        ],
        viral: [
          `The ${n} system all small businesses are starting to copy`,
          `Why ${n} businesses with content systems get more leads than those who post more`,
          `Everyone talks about ${n} content — this is what actually builds solid reputation`,
          `The shift from random ${n} results to repeatable process everyone should know`,
        ],
      },
    };
  ...
  if (isMediana)
    return {
      ...
      objectivePool: [
        "Optimize team conversion rate",
        "Improve follow-up and CRM process",
        "Identify bottlenecks in conversion",
        "Automate manual steps blocking sales",
      ],
      topicMap: {
        curiosity: [
          `What your ${n} CRM is telling you that you're not reading`,
          `Why your ${n} team has activity but low actual conversion`,
          `The ${n} data point that shows where opportunities are lost`,
          `What separates the ${n} sales process that converts from the one that doesn't`,
        ],
        pain: [
          `Why your ${n} team works more but converts the same or less`,
          `Why your ${n} loses opportunities in the follow-up stage`,
          `${n} CRM not giving useful data? Here's the real problem`,
          `The conversion bottleneck every ${n} with a team has`,
        ],
        story: [
          `How we optimized our ${n} follow-up and conversion rose significantly`,
          `What we implemented in our ${n} CRM that changed this month's numbers`,
          `How we automated ${n} follow-up and the team converted more without extra effort`,
          `The ${n} conversion diagnosis that found where 30% of opportunities were lost`,
        ],
        authority: [
          `The CRM process for ${n} teams that improves conversion without more effort`,
          `How to design a ${n} follow-up system that recovers lost opportunities`,
          `The ${n} metrics that actually predict this month's conversion result`,
          `The ${n} automation flow that eliminates manual steps blocking sales`,
        ],
        mistake: [
          `The CRM mistake ${n} teams make that loses conversion`,
          `Why your ${n} team isn't following up at the right moment`,
          `The automation missing from your ${n} process is costing clients every week`,
          `Stop measuring this in your ${n} — it doesn't predict real team conversion`,
        ],
        opportunity: [
          `The ${n} automation your team can implement this week to improve conversion`,
          `Why now is the time to audit your ${n} follow-up process`,
          `The ${n} CRM angle your competition with a team isn't optimizing yet`,
          `The ${n} conversion optimization window that closes with every passing week`,
        ],
        viral: [
          `The ${n} process changing how teams manage conversion`,
          `Why ${n} businesses with optimized teams convert more with the same effort`,
          `Everyone talks about CRM for ${n} — this is what actually moves team conversion`,
          `The ${n} automation change that improved conversion numbers this month`,
        ],
      },
    };
  ...
  if (isGrande)
    return {
      ...
      objectivePool: [
        "Scale with brand consistency across departments",
        "Optimize profitability at scale",
        "Improve multi-channel campaign coordination",
        "Inform executive decisions with clear metrics",
      ],
      topicMap: {
        curiosity: [
          `The ${n} executive dashboard that shows where profitability is lost at scale`,
          `What brand consistency in ${n} across departments actually requires`,
          `Why ${n} multi-channel campaigns lose effectiveness without this system`,
          `The ${n} executive data leadership isn't reading that's costing margin`,
        ],
        pain: [
          `Why your ${n} operation grows but profitability doesn't scale equally`,
          `Why ${n} brand consistency dilutes across departments at scale`,
          `${n} campaigns without channel coherence? Here's the systemic cause`,
          `The scalability problem large ${n} organizations with multiple teams have`,
        ],
        story: [
          `How we redesigned our ${n} system to scale without losing brand consistency`,
          `What we built into our ${n} dashboard that changed executive leadership decisions`,
          `How we coordinated ${n} campaigns across departments without losing brand coherence`,
          `The ${n} executive diagnosis that revealed where profitability was lost at scale`,
        ],
        authority: [
          `The executive dashboard system for ${n} organizations already scaling`,
          `How to build ${n} brand consistency across departments at scale`,
          `The ${n} executive metrics that actually inform leadership decisions`,
          `The ${n} multi-channel model that maintains coherence at corporate scale`,
        ],
        mistake: [
          `The scalability mistake large ${n} organizations make that loses profitability`,
          `Why your ${n} loses brand consistency across departments when scaling`,
          `The ${n} campaign coordination gap costing margin every month`,
          `Stop measuring this in your ${n} at the executive level — it doesn't inform real decisions`,
        ],
        opportunity: [
          `The ${n} profitability optimization window at scale leadership isn't using`,
          `Why now is the time to audit your ${n} brand consistency across departments`,
          `The ${n} dashboard system your corporate competition hasn't implemented yet`,
          `The ${n} operational efficiency opportunity at scale nobody is capitalizing on`,
        ],
        viral: [
          `The ${n} corporate model changing how executives manage scale`,
          `Why ${n} organizations that scale without losing profitability have this system in common`,
          `Everyone talks about ${n} scalability — this is what actually maintains brand consistency`,
          `The ${n} leadership change that improved cross-department coherence is redefining scale`,
        ],
      },
    };
  ...
  return {
    ...
    objectivePool: [
      "Get local clients this week",
      "Activate WhatsApp as direct sales channel",
      "Convert one-time clients into repeat customers",
      "Increase daily local visibility on low budget",
    ],
    topicMap: {
      curiosity: [
        `The channel that gives ${n} micro businesses the most local clients per week`,
        `Why WhatsApp converts better than Instagram for your local ${n}`,
        `What your ${n} competition doesn't know about daily local visibility`,
        `The repeat client secret in ${n} nobody explains`,
      ],
      pain: [
        `Why your local ${n} has low traffic even when you do things right`,
        `Why your ${n} clients don't come back without you chasing them`,
        `Still no clients this week? What happens to most local ${n} businesses`,
        `The daily visibility problem blocking every ${n} micro business`,
      ],
      story: [
        `How I got more ${n} clients this week using only WhatsApp`,
        `What I changed in my ${n} to have repeat clients without chasing them`,
        `The message that brought new local clients to my ${n} this week`,
        `How I made my local ${n} the visible option in the neighborhood`,
      ],
      authority: [
        `The WhatsApp system for ${n} micro businesses that delivers clients this week`,
        `How to build local ${n} visibility on a daily low budget`,
        `What local ${n} businesses that always have repeat clients do differently`,
        `The ${n} referral strategy that works without spending on advertising`,
      ],
      mistake: [
        `The WhatsApp mistake local ${n} businesses make that loses weekly sales`,
        `Why your ${n} has followers but no clients this week`,
        `Why your ${n} clients don't refer you without being asked`,
        `Stop doing this in your local ${n} — it's blocking local sales`,
      ],
      opportunity: [
        `The local ${n} visibility opportunity your competition isn't taking`,
        `The window to capture ${n} clients with WhatsApp that closes soon`,
        `The local ${n} niche nobody is dominating in your area yet`,
        `The ${n} repeat client window that closes if you don't act this week`,
      ],
      viral: [
        `The WhatsApp message for local ${n} everyone is starting to copy`,
        `Why local ${n} businesses are changing this to get more clients this week`,
        `Everyone talks about ${n} marketing — this is what actually gives local clients`,
        `The local ${n} change generating the most repeat clients on a low budget`,
      ],
    },
  };
}
...
function buildAudienceIntelligence(
  n,
  au,
  au1,
  pr,
  ...
  const profile = industryProfile(n, isES);
  const [k0 = "resultados", k1 = "confianza", k2 = "crecimiento"] =
    profile.keywords;
...
app.post("/api/script/generate", (req, res) => {
  ...
  if (!niche.trim() || !product.trim() || !audience.trim()) {
    return res
      .status(400)
      .json({ error: "niche, product, and audience are required" });
  }
  ...
  const au1 =
    _auSubj && _auSubj !== au ? _auSubj.split(/\s+/)[0] : au.replace(/s$/i, "");
  ...
  const tr = buildTransformation(n, pr, isES);
  const si = buildStageIntelligence(businessStage, n, pr, au, isES);
  const intel = buildAudienceIntelligence(
    n,
    au,
    au1,
    pr,
  ...
  const rawOutput = {
    ...
    script:
      (activeScriptMap[hookType] ?? activeScriptMap.curiosity) +
      si.scriptContext,
  };
  ...
  const title = ctxApply(
    pick(activeTitleMap[hookType] ?? activeTitleMap.curiosity),
    n,
    au,
    pr,
    isES,
  ...
  const purity = audiencePurityScan(
    { ...body, cta, title },
    lockInfo.blacklist,
  );
  console.log(
  ...
  res.json({
    ...
    title,
    hashtags,
    audienceConsistencyScore: purity.score,
    stageLabel: si.label,
    stageVocabulary: si.vocabulary,
  });
});
...
app.post("/api/content-strategy/generate", (req, res) => {
    ...
    sales: [
      {
        ...
        note: `Surface the core problem ${au || "your audience"} hasn't fully named yet — problem awareness unlocks purchase intent.`,
      },
      {
        day: "Tuesday",
        hookType: "authority",
        contentType: "Social Proof",
        note: "Establish credibility before the mid-week high-intent window with proof-driven content.",
      },
      {
        day: "Wednesday",
        hookType: "opportunity",
        contentType: "Product Demo",
        note: `Mid-week captures peak purchase intent — present ${pr || "your offer"} directly and clearly.`,
      },
      {
        day: "Thursday",
        hookType: "story",
        contentType: "Story",
        note: "Human narrative after direct sales content maintains trust and reduces resistance.",
      },
      {
        day: "Friday",
        hookType: "pain",
        contentType: "Listicle",
        note: "End-of-week pain reinforcement with a clear, low-friction CTA.",
      },
      {
        day: "Saturday",
        hookType: "curiosity",
        contentType: "Q&A",
        note: "Curiosity-driven community content keeps algorithm engagement warm over the weekend.",
      },
      {
        day: "Sunday",
        hookType: "authority",
        contentType: "Educational",
        note: `Pre-week authority content primes ${au || "your audience"} before Monday's peak intent window.`,
      },
    ],
    leads: [
      {
        ...
        hookType: "curiosity",
        contentType: "Educational",
        note: `Open the week with an insight gap — ${au || "your audience"} in a learning mindset is most likely to follow a link.`,
      },
      {
        day: "Tuesday",
        hookType: "pain",
        contentType: "Listicle",
        note: "Name the specific pain before offering the solution. Problem-aware leads are the easiest to convert.",
      },
      {
        day: "Wednesday",
        hookType: "authority",
        contentType: "Social Proof",
        note: "Proof mid-week reduces friction for link-in-bio clicks and DM inquiries.",
      },
      {
        day: "Thursday",
        hookType: "story",
        contentType: "Story",
        note: "Story builds emotional trust — Thursday is the strongest day for DM inquiries and opt-ins.",
      },
      {
        day: "Friday",
        hookType: "curiosity",
        contentType: "Q&A",
        note: "Engage the audience with an unresolved question that your lead magnet answers.",
      },
      {
        day: "Saturday",
        hookType: "opportunity",
        contentType: "Trending",
        note: "Weekend trend content expands reach to cold audiences who haven't found you yet.",
      },
      {
        day: "Sunday",
        hookType: "pain",
        contentType: "Educational",
        note: "Sunday reflection content connects with problem-aware audiences planning their next move.",
      },
    ],
    brand_awareness: [
      {
        ...
        note: "Start the week with authenticity — behind-the-scenes content builds long-term connection.",
      },
      {
        day: "Tuesday",
        hookType: "curiosity",
        contentType: "Educational",
        note: "Curiosity-driven value content earns shares from new audiences who didn't follow you yet.",
      },
      {
        day: "Wednesday",
        hookType: "viral",
        contentType: "Trending",
        note: "Mid-week trend content maximizes algorithmic reach — peak sharing day for most niches.",
      },
      {
        day: "Thursday",
        hookType: "authority",
        contentType: "Listicle",
        note: `Establish ${n} credibility before the weekend engagement window with shareable frameworks.`,
      },
      {
        day: "Friday",
        hookType: "story",
        contentType: "Challenge",
        note: "Interactive challenge content drives community participation and profile visits.",
      },
      {
        day: "Saturday",
        hookType: "viral",
        contentType: "Hot Take",
        note: "Weekend hot takes generate comments and shares from audiences who don't follow you yet.",
      },
      {
        day: "Sunday",
        hookType: "curiosity",
        contentType: "Q&A",
        note: "Community Q&A deepens connection with existing followers and improves retention.",
      },
    ],
    authority: [
      {
        ...
        hookType: "authority",
        contentType: "Educational",
        note: `Lead with expertise — Monday audiences are in a learning mindset and reward genuine ${n} insight.`,
      },
      {
        day: "Tuesday",
        hookType: "mistake",
        contentType: "Listicle",
        note: "Mistake content signals deep domain knowledge. Creates saves — the highest-trust engagement signal.",
      },
      {
        day: "Wednesday",
        hookType: "story",
        contentType: "Story",
        note: "Personal narrative humanizes your expertise and creates emotional trust alongside intellectual credibility.",
      },
      {
        day: "Thursday",
        hookType: "authority",
        contentType: "Social Proof",
        note: "Case studies and outcome content on Thursday reinforce the credibility established earlier in the week.",
      },
      {
        day: "Friday",
        hookType: "curiosity",
        contentType: "Educational",
        note: "End-of-week insight creates anticipation and positions you as the go-to source before the weekend.",
      },
      {
        day: "Saturday",
        hookType: "mistake",
        contentType: "Hot Take",
        note: "Contrarian weekend content drives discussion and introduces your perspective to new audiences.",
      },
      {
        day: "Sunday",
        hookType: "story",
        contentType: "Behind the Scenes",
        note: "Behind-the-scenes content builds relationship depth with your core authority audience.",
      },
    ],
    engagement: [
      {
        ...
        hookType: "curiosity",
        contentType: "Q&A",
        note: `Start with a question — Monday audiences respond strongly to participation-driven content.`,
      },
      {
        day: "Tuesday",
        hookType: "mistake",
        contentType: "Hot Take",
        note: "Mistake content generates comments from people who've experienced the same situation.",
      },
      {
        day: "Wednesday",
        hookType: "viral",
        contentType: "Challenge",
        note: "Mid-week challenges drive the highest participation rates of any content format.",
      },
      {
        day: "Thursday",
        hookType: "story",
        contentType: "Story",
        note: "Story content builds the emotional equity that drives saves, shares, and DMs.",
      },
      {
        day: "Friday",
        hookType: "viral",
        contentType: "Trending",
        note: "Trending content on Friday captures weekend sharing behavior at its peak.",
      },
      {
        day: "Saturday",
        hookType: "curiosity",
        contentType: "Behind the Scenes",
        note: "Saturday audiences engage more deeply with authentic, unpolished content.",
      },
      {
        day: "Sunday",
        hookType: "story",
        contentType: "Q&A",
        note: "Community conversation on Sunday builds anticipation and strengthens following loyalty.",
      },
    ],
  };
  const weeklyMapES = {
    sales: [
      {
        ...
        note: `Expón el problema central que ${au || "tu audiencia"} todavía no ha nombrado del todo — la conciencia del problema desbloquea la intención de compra.`,
      },
      {
        day: "Martes",
        hookType: "authority",
        contentType: "Prueba Social",
        note: "Establece credibilidad antes de la ventana de alta intención de mitad de semana con contenido basado en pruebas.",
      },
      {
        day: "Miércoles",
        hookType: "opportunity",
        contentType: "Demo del Producto",
        note: `Mitad de semana captura el pico de intención de compra — presenta ${pr || "tu oferta"} directamente y con claridad.`,
      },
      {
        day: "Jueves",
        hookType: "story",
        contentType: "Historia",
        note: "La narrativa humana después del contenido de ventas directo mantiene la confianza y reduce la resistencia.",
      },
      {
        day: "Viernes",
        hookType: "pain",
        contentType: "Lista",
        note: "Refuerzo de dolor de fin de semana con un CTA claro y de baja fricción.",
      },
      {
        day: "Sábado",
        hookType: "curiosity",
        contentType: "Preguntas y Respuestas",
        note: "Contenido de comunidad impulsado por curiosidad mantiene el engagement algorítmico activo durante el fin de semana.",
      },
      {
        day: "Domingo",
        hookType: "authority",
        contentType: "Educativo",
        note: `Contenido de autoridad de pre-semana prepara a ${au || "tu audiencia"} antes de la ventana de mayor intención del lunes.`,
      },
    ],
    leads: [
      {
        ...
        hookType: "curiosity",
        contentType: "Educativo",
        note: `Abre la semana con una brecha de insight — ${au || "tu audiencia"} en mentalidad de aprendizaje es más probable que haga clic en un enlace.`,
      },
      {
        day: "Martes",
        hookType: "pain",
        contentType: "Lista",
        note: "Nombra el dolor específico antes de ofrecer la solución. Los prospectos conscientes del problema son los más fáciles de convertir.",
      },
      {
        day: "Miércoles",
        hookType: "authority",
        contentType: "Prueba Social",
        note: "La prueba a mitad de semana reduce la fricción para clics en bio y consultas por DM.",
      },
      {
        day: "Jueves",
        hookType: "story",
        contentType: "Historia",
        note: "La historia genera confianza emocional — el jueves es el día más fuerte para consultas y suscripciones.",
      },
      {
        day: "Viernes",
        hookType: "curiosity",
        contentType: "Preguntas y Respuestas",
        note: "Involucra a la audiencia con una pregunta no resuelta que responde tu lead magnet.",
      },
      {
        day: "Sábado",
        hookType: "opportunity",
        contentType: "Tendencia",
        note: "El contenido de tendencia del fin de semana expande el alcance a audiencias frías.",
      },
      {
        day: "Domingo",
        hookType: "pain",
        contentType: "Educativo",
        note: "El contenido de reflexión dominical conecta con audiencias conscientes del problema que planean su próximo movimiento.",
      },
    ],
    brand_awareness: [
      {
        day: "Lunes",
        hookType: "story",
        contentType: "Detrás de Escena",
        note: "Empieza la semana con autenticidad — el contenido detrás de escena construye conexión a largo plazo.",
      },
      {
        day: "Martes",
        hookType: "curiosity",
        contentType: "Educativo",
        note: "El contenido de valor impulsado por curiosidad gana compartidos de nuevas audiencias que aún no te siguen.",
      },
      {
        day: "Miércoles",
        hookType: "viral",
        contentType: "Tendencia",
        note: "El contenido de tendencia de mitad de semana maximiza el alcance algorítmico — el día de mayor compartición en la mayoría de nichos.",
      },
      {
        day: "Jueves",
        hookType: "authority",
        contentType: "Lista",
        note: `Establece credibilidad en ${n} antes de la ventana de engagement del fin de semana con marcos compartibles.`,
      },
      {
        day: "Viernes",
        hookType: "story",
        contentType: "Desafío",
        note: "El contenido de desafío interactivo impulsa la participación de la comunidad y visitas al perfil.",
      },
      {
        day: "Sábado",
        hookType: "viral",
        contentType: "Opinión Fuerte",
        note: "Las opiniones fuertes del fin de semana generan comentarios y compartidos de audiencias que aún no te siguen.",
      },
      {
        day: "Domingo",
        hookType: "curiosity",
        contentType: "Preguntas y Respuestas",
        note: "Las preguntas y respuestas de comunidad profundizan la conexión con seguidores existentes y mejoran la retención.",
      },
    ],
    authority: [
      {
        ...
        hookType: "authority",
        contentType: "Educativo",
        note: `Lidera con expertise — las audiencias del lunes están en mentalidad de aprendizaje y recompensan el insight genuino de ${n}.`,
      },
      {
        day: "Martes",
        hookType: "mistake",
        contentType: "Lista",
        note: "El contenido de errores señala conocimiento profundo del dominio. Genera guardados — la señal de engagement de mayor confianza.",
      },
      {
        day: "Miércoles",
        hookType: "story",
        contentType: "Historia",
        note: "La narrativa personal humaniza tu expertise y genera confianza emocional junto a la credibilidad intelectual.",
      },
      {
        day: "Jueves",
        hookType: "authority",
        contentType: "Prueba Social",
        note: "Los casos de estudio del jueves refuerzan la credibilidad establecida a principios de semana.",
      },
      {
        day: "Viernes",
        hookType: "curiosity",
        contentType: "Educativo",
        note: "El insight de fin de semana genera anticipación y te posiciona como la fuente de referencia antes del fin de semana.",
      },
      {
        day: "Sábado",
        hookType: "mistake",
        contentType: "Opinión Fuerte",
        note: "El contenido contrario del fin de semana genera discusión e introduce tu perspectiva a nuevas audiencias.",
      },
      {
        day: "Domingo",
        hookType: "story",
        contentType: "Detrás de Escena",
        note: "El contenido detrás de escena construye profundidad de relación con tu audiencia central de autoridad.",
      },
    ],
    engagement: [
      {
        ...
        hookType: "curiosity",
        contentType: "Preguntas y Respuestas",
        note: `Empieza con una pregunta — las audiencias del lunes responden fuerte al contenido participativo.`,
      },
      {
        day: "Martes",
        hookType: "mistake",
        contentType: "Opinión Fuerte",
        note: "El contenido de errores genera comentarios de personas que vivieron la misma situación.",
      },
      {
        day: "Miércoles",
        hookType: "viral",
        contentType: "Desafío",
        note: "Los desafíos de mitad de semana generan las tasas de participación más altas de cualquier formato.",
      },
      {
        day: "Jueves",
        hookType: "story",
        contentType: "Historia",
        note: "El contenido de historia construye el capital emocional que impulsa guardados, compartidos y DMs.",
      },
      {
        day: "Viernes",
        hookType: "viral",
        contentType: "Tendencia",
        note: "El contenido de tendencia del viernes captura el comportamiento de compartición del fin de semana en su pico.",
      },
      {
        day: "Sábado",
        hookType: "curiosity",
        contentType: "Detrás de Escena",
        note: "Las audiencias del sábado se involucran más profundamente con contenido auténtico y sin pulir.",
      },
      {
        day: "Domingo",
        hookType: "story",
        contentType: "Preguntas y Respuestas",
        note: "La conversación de comunidad del domingo construye anticipación y fortalece la lealtad de los seguidores.",
      },
    ],
  };
  ...
  const postingMap = {
    sales: {
      recommended: "5x_week",
      reason: `Sales conversion requires consistent presence in the feed across the purchase decision window. 5x/week maintains visibility without overwhelming — ${au || "your audience"} needs 5–7 exposures to a solution before taking action.`,
    },
    leads: {
      recommended: "5x_week",
      reason: `Lead generation is a volume and consistency game. 5x/week gives you enough touchpoints to move ${au || "your audience"} from problem-aware to opt-in ready within a single week cycle.`,
    },
    brand_awareness: {
      recommended: "daily",
      reason: `Brand awareness scales with frequency. Daily posting in ${n} maximizes algorithmic surface area and creates the repetition that turns passive viewers into active followers.`,
    },
    authority: {
      recommended: "3x_week",
      reason: `Authority is built on depth, not volume. 3x/week gives you the production time to create content that genuinely demonstrates expertise — which is more valuable than daily shallow content for ${n} authority positioning.`,
    },
    engagement: {
      recommended: "daily",
      reason: `Engagement compounds with frequency. Daily content gives ${au || "your audience"} more opportunities to participate, and the algorithm rewards engagement velocity, which requires consistent touchpoints.`,
    },
  };
  const postingMapES = {
    sales: {
      recommended: "5x_week",
      reason: `Las ventas requieren presencia consistente durante la ventana de decisión de compra. 5 veces/semana mantiene visibilidad sin saturar — ${au || "tu audiencia"} necesita entre 5 y 7 exposiciones a una solución antes de actuar.`,
    },
    leads: {
      recommended: "5x_week",
      reason: `La generación de prospectos es un juego de volumen y consistencia. 5 veces/semana da suficientes puntos de contacto para mover a ${au || "tu audiencia"} de consciente del problema a listo para optar dentro de un ciclo de semana.`,
    },
    brand_awareness: {
      recommended: "daily",
      reason: `La conciencia de marca escala con frecuencia. Publicar diariamente en ${n} maximiza la superficie algorítmica y crea la repetición que convierte espectadores pasivos en seguidores activos.`,
    },
    authority: {
      recommended: "3x_week",
      reason: `La autoridad se construye con profundidad, no con volumen. 3 veces/semana te da tiempo de producir contenido que realmente demuestra expertise — lo que vale más que contenido diario superficial para el posicionamiento de autoridad en ${n}.`,
    },
    engagement: {
      recommended: "daily",
      reason: `El engagement se compone con frecuencia. El contenido diario da a ${au || "tu audiencia"} más oportunidades de participar, y el algoritmo recompensa la velocidad de engagement, que requiere puntos de contacto consistentes.`,
    },
  };
...
const ENHANCE_EQ = {
  ...
  deep_clean: { brightness: 0.04, contrast: 1.14, saturation: 1.1, gamma: 1.0 },
  cinematic: {
    brightness: -0.03,
    contrast: 1.28,
    saturation: 0.78,
  ...
  low_light: { brightness: 0.2, contrast: 1.22, saturation: 1.15, gamma: 1.65 },
  audio_cleaner: null,
};
...
function buildEnhanceFilters(
  ...
  const needsHeavyClean =
    preset === "deep_clean" ||
    preset === "low_light" ||
    noiseStrength === "medium" ||
    noiseStrength === "high" ||
    noiseStrength === "extreme";
  if (needsHeavyClean) {
  ...
  if (toggles.colorCorrection && eq.gamma !== 1.0)
    expParts.push(`gamma=${eq.gamma}`);
  if (expParts.length) filters.push(`eq=${expParts.join(":")}`);

  // ── 3.5. Highlight Roll-Off 66 ───────────────────────────────────────────────
  ...
  if (preset !== "cinematic") {
    filters.push(
      "curves=all='0/0 0.5/0.5 0.72/0.72 0.82/0.79 0.92/0.87 1/0.92'",
    );
  }
...
function buildFacePreserveComplex(
  ...
  const bgDenoise =
    preset === "deep_clean"
      ? "hqdn3d=3:3:8:8"
      : toggles.noiseReduction
        ? `hqdn3d=${HQDN3D[noiseStrength] ?? HQDN3D.medium}`
        : preset === "low_light"
          ? `hqdn3d=${HQDN3D.medium}`
          : null;
  ...
  if (preset !== "cinematic") {
    post.push(
      "curves=all='0/0 0.50/0.50 0.72/0.72 0.82/0.79 0.92/0.87 1/0.92'",
    );
  }
  // Studio Look
  if (studioLook && studioLook !== "off") {
...
app.post(
  ...
  async (req, res) => {
    if (!FFMPEG) {
      return res
        .status(500)
        .json({ error: "FFmpeg not available on this server." });
    }
    ...
    try {
      ...
      if (srcProbe) {
        ...
        res.setHeader(
          "X-Vyron-Src-Duration",
          String(srcProbe.durationSec ?? 0),
        );
      ...
      if (!res.headersSent) {
        res
          .status(500)
          .json({ error: err?.message ?? "Preview conversion failed" });
      }
    }
  },
);
    ...
    try {
      ...
      await execFileAsync(
        ...
        [
          "-y",
          "-ss",
          "1", // fast-seek to 1 s (avoids black frames at start)
          "-i",
          inputPath,
          "-frames:v",
          "1", // extract exactly one frame
          "-vf",
          "scale=640:-2", // max 640 px wide, even height
          "-f",
          "image2",
          "-q:v",
          "3", // good JPEG quality
  ...
  async (req, res) => {
    ...
    if (
      ...
      if (
        ...
        ny >= 0 &&
        nw >= 0.08 &&
        nh >= 0.08 && // too small = noise / false positive
        nx + nw <= 1.0 &&
        ny + nh <= 1.0 // must stay within frame
      ) {
    ...
    try {
      ...
      const baseFilters = buildEnhanceFilters(
        preset,
        toggles,
        noiseStrength,
        "off",
      ...
      if (facePreserveActive) {
        ...
        const complexGraph = buildTeethWhiteningComplex(
          baseFilters,
          teethWhitening,
        );
        args.push("-filter_complex", complexGraph);
...
app.post("/api/export/mp4", async (req, res) => {
  ...
  if (!FFMPEG) {
    return res.status(500).json({
      error:
        "FFmpeg not found on this server. Set the FFMPEG_PATH environment variable to the absolute path of the ffmpeg binary.",
    });
  }
  if (!videoUrl?.trim()) {
  ...
  try {
    ...
    writeFileSync(
      ...
      buildAssFile(
        subtitles,
        preset,
        Number(subtitleScale) || 1.0,
        Number(subtitlePosition) || 0,
      ),
      "utf-8",
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="subtitles-${ymd}.mp4"`,
    );
